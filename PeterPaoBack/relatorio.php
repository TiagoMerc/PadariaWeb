...
function mask($val, $mask) {
        ...
        if($_POST["tipoBusca"] == "vendas"){
            if (isset($_POST["search"])) {
                ...
                if (strcmp($opcao, "Código") == 0) {
                    $query = "SELECT * FROM vendas_cod_prod('$consulta', '$dataInicio', '$dataFim');";
                } else if (strcmp($opcao, "Nome") == 0) {
                    $query = "SELECT * FROM vendas_nome_prod('$consulta%', '$dataInicio', '$dataFim');";
                } else {
                    $query = "SELECT * FROM vendas_categoria_prod('$consulta', '$dataInicio', '$dataFim');";
                }   
    ...
    if($_GET["tipoBusca2"] == "produz"){
                if (isset($_GET["search2"])) {
                    ...
                    $opcao = $_GET["buscaOp2"]; 
                    if (strcmp($opcao, "Código") == 0) {
                        $query = "SELECT * FROM producao_cod_prod($consulta, '$dataInicio', '$dataFim')";
                    } else if (strcmp($opcao, "Nome") == 0) {
                       $query = "SELECT * FROM producao_nome_prod('$consulta%', '$dataInicio', '$dataFim')";
                    } else {
                        $query = "SELECT * FROM producao_categoria_prod('$consulta', '$dataInicio', '$dataFim')";
                    }   
    ...
    if (isset($_GET["pordata3"])) {
        ...
        if($tipo == "vendas"){
            ...
            while($i<$numregs)
            {
                ...
                $preco=pg_result($result2,$i,'preco_unitario');
                $qtd_pedido=pg_result($result2,$i,'quantidade_pedidos');
                $qtd_total=pg_result($result2,$i,'quantidade');
                $vendas = $qtd_total * $preco;
                $vendas = money_format('%.2n',$vendas); 
                $preco = money_format('%.2n',$preco);
                                    ...
                                    <div id="consultar">
                                        ...
                                        <table id="tabelaConsultar">
                                            ...
                                            <tbody>
                                              <?php
                                                    ...
                                                    <td style="text-align:left;"><?php echo $obj->nome_prod;?></td>    
                                                    <td><?php echo $obj->quantidade_estoque; ?></td>
                                                    <td style="text-align:left; text-indent: 17px;"><?php echo $obj->categoria; ?></td>
                                                    <td style="text-align:left;"><?php echo $obj->descricao; ?></td>
                                                   <td style="text-align:left; text-indent: 17px;"><?php echo "R$ " . money_format('%.2n', $obj->preco_unitario); ?></td> 
                                                    <td><?php echo $obj->quantidade_pedidos; ?></td>    
                                                    <?php         
                                                    }
                                                    ?>         
                                     ...
                                     <div id="adicionar2">
                                        ...
                                        <form class="busca">
                                          ...
                                          <input type="text" placeholder="buscar por código.." maxlength="10" name="search2" oninput="replace_not_number('2')" pattern="\d{1,10}" title="Apenas números. 1 a 10 dígitos." required>
                                        ...
                                        <table id="tabelaProducao">
                                            ...
                                            <tbody>
                                               <?php
                                                    ...
                                                    <td><?php echo $obj->quantidade_estoque; ?></td>
                                                    <td style="text-align:left; text-indent: 17px;"><?php echo $obj->categoria; ?></td>
                                                    <td class="alinhamento"><?php echo $obj->descricao; ?></td>
                                                    <td style="text-align:left; text-indent: 17px;"><?php echo "R$ " . money_format('%.2n', $obj->preco_unitario); ?></td> 
...
<script>
    ...
    function muda_busca2(){
        ...
        if(opcao.localeCompare("Código") == 0){
            $("input[name='search2']").replaceWith("<input type=\"text\" placeholder=\"buscar por código..\" maxlength=\"10\" name=\"search2\" oninput=\"replace_not_number('1')\" pattern=\"\\d{1,10}\" title=\"Apenas números. 1 a 10 dígitos.\" required>");
            $("select[name='search2']").replaceWith("<input type=\"text\" placeholder=\"buscar por código..\" maxlength=\"10\" name=\"search2\" oninput=\"replace_not_number('1')\" pattern=\"\\d{1,10}\" title=\"Apenas números. 1 a 10 dígitos.\" required>");
        }else if(opcao.localeCompare("Nome") == 0){
            $("input[name='search2']").replaceWith("<input type=\"text\" placeholder=\"busca por nome..\" maxlength=\"50\" name=\"search2\"  title=\"Apenas letras\" required>");
            $("select[name='search2']").replaceWith("<input type=\"text\" placeholder=\"busca por nome..\" maxlength=\"50\" name=\"search2\"  title=\"Apenas letras\" required>");            
        }else if(opcao.localeCompare("Categoria") == 0){
            $("input[name='search2']").replaceWith( "<select name= \"search2\" id=\"buscaOp\"><option value=\"Pães\">Pães</option> <option value=\"Tortas\">Tortas</option> <option value=\"Bolos\">Bolos</option> <option value=\"Doces\">Doces</option> <option value=\"Bebidas\">Bebidas</option> <option value=\"Ingrediente\">Ingrediente</option></select> ");
        }
    }
